...
def home_def():
    ...
    nselist = Path.joinpath(BASE_DIR,'data/NSE.csv')
    nse=pd.read_csv(nselist)
    symbols=np.array(nse["Symbol"])
   
    symboll = st.sidebar.selectbox('Select Stock for prediction', symbols)
    ...
    forecasting,overview, fundamental,technical = st.tabs(['**Forecasting**','Overview','Fundamental','Technical'])
    ...
    with forecasting:
        ...
        # Predict forecast with Prophet Model
